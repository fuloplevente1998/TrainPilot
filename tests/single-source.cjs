// Execute the real shipped app.js, independently of legacy section tests.
...
assert.deepEqual(order,['app.js']);
assert.deepEqual(fs.readFileSync('www/runtime-manifest.txt','utf8').trim().split(/\r?\n/),['app.js']);
assert.deepEqual(fs.readdirSync('www').filter(x=>x.endsWith('.js')).sort(),['app.js','sw.js']);
assert.equal(run('rf263ParseDate("2023-02-29")'),null);
