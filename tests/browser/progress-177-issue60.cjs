...
(async()=>{await new Promise(r=>server.listen(0,'127.0.0.1',r));let browser;try{
 ...
 assert.match(text,/Átlagos edzésidő/);
 assert.match(text,/Összes volumen.*kg/);
 assert.equal(await page.locator('#tp177VolumeTrend').isHidden(),true,'volume chart starts collapsed');
 await page.locator('.tp177-volume-toggle').click();
 assert.equal(await page.locator('#tp177VolumeTrend').isVisible(),true,'volume chart opens inside total-volume card');
 assert.ok((await page.locator('.tp177-chart').boundingBox()).height<140,'expanded chart stays compact');
 assert.equal(await page.locator('.tp177-chart-date').count(),13,'chart dates remain visible');
 ...
 assert.ok(vertical.railH>80&&vertical.barH>0&&vertical.barW<=40,'volume must render as vertical columns: '+JSON.stringify(vertical));
 ...
 assert.match(await page.locator('#tp177MetricDetails').innerText(),/Átlagos edzésidő.*60 perc/s);
