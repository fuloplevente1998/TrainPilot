...
(async()=>{
 ...
 try{
  ...
  for(const [w,h] of sizes)for(const [width,height] of [[w,h],[h,w]]){
   ...
   const libraryArrowStyle=async()=>firstLibraryCard.locator('summary').evaluate(e=>{const s=getComputedStyle(e,'::after');return {border:s.borderTopWidth,bg:s.backgroundImage,color:s.color,content:s.content,width:parseFloat(s.width),height:parseFloat(s.height),clip:s.clipPath,transform:s.transform}});
   const libraryClosed=await libraryArrowStyle();
   assert.equal(libraryClosed.border,'0px','exercise disclosure chevron must be borderless');
   assert.equal(libraryClosed.bg,'none','exercise disclosure must not regain the legacy gradient box');
   assert.ok(libraryClosed.width>=26&&libraryClosed.width<=34&&libraryClosed.height>=32&&libraryClosed.height<=40,'exercise disclosure chevron must match the global dimensions: '+JSON.stringify(libraryClosed));
   assert.ok(libraryClosed.content.includes('›'),'exercise disclosure must render the global chevron, not a CSS triangle: '+JSON.stringify(libraryClosed));
   assert.equal(libraryClosed.clip,'none','exercise disclosure must not retain a triangle clipping mask');
   ...
   assert.notEqual(libraryOpen.transform,libraryClosed.transform,'exercise disclosure chevron must rotate from right to down when opened');
   ...
   const historyArrowStyle=async()=>historyArrow.evaluate(e=>{const s=getComputedStyle(e),b=getComputedStyle(e,'::before');return {font:parseFloat(s.fontSize),border:s.borderTopWidth,bg:s.backgroundImage,width:parseFloat(s.width),height:parseFloat(s.height),fill:b.backgroundColor,clip:b.clipPath,glyph:b.content,transform:s.transform}});
   const historyClosed=await historyArrowStyle();
   assert.equal(historyClosed.border,'0px','journal disclosure chevron must be borderless');
   assert.equal(historyClosed.bg,'none','journal disclosure must not regain legacy gradient');
   assert.ok(historyClosed.width>=25&&historyClosed.height>=30,'journal disclosure must have shared chevron dimensions: '+JSON.stringify(historyClosed));
   assert.ok(historyClosed.glyph.includes('›'),'journal disclosure must use global chevron');
   assert.equal(historyClosed.clip,'none','journal disclosure must not use triangle clipping');
   ...
   assert.notEqual(historyOpen.transform,historyClosed.transform,'journal shared chevron must rotate when opened');
   ...
   const healthArrowTransform=async()=>healthArrow.evaluate(e=>({glyph:getComputedStyle(e,'::before').content,host:getComputedStyle(e).transform}));
   ...
   assert.ok((await healthArrowTransform()).glyph.includes('›'),'Health disclosure must use the shared chevron');
   await healthToggle.click();await page.waitForTimeout(180);
