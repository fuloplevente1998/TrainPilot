...
const result=await p.evaluate(()=>{
 ...
 return {history:merged.history.length,weights:merged.weights.length,historyIds:merged.history.map(x=>x.id),weightIds:merged.weights.map(x=>x.id),againHistory:again.history.length,againWeights:again.weights.length};
});
assert.equal(result.history,2);assert.equal(result.weights,2);assert.equal(result.againHistory,2);assert.equal(result.againWeights,2);assert.ok(result.historyIds.every(x=>/^tp18-h-/.test(x)));assert.ok(result.weightIds.every(x=>/^tp18-w-/.test(x)));
