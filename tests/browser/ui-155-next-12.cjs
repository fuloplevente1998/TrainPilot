...
(async()=>{
 ...
 try{
  ...
  const themeDropdown=page.locator('.tp162-theme-dropdown');assert.equal(await page.locator('#tp155ThemePicker').count(),0,'theme chooser must stay inline instead of opening the legacy modal');
  assert.ok(await themeDropdown.locator('.tp155-theme-option').count()>=10,'inline theme chooser must expose the complete theme set');
  assert.equal(await themeDropdown.locator('.tp162-theme-column').count(),2,'inline theme chooser must use separate Basic and Vivid columns');
  assert.equal(await themeDropdown.locator('.tp155-theme-list').evaluate(e=>getComputedStyle(e).overflowY),'visible','inline theme chooser must not clip its theme list');
  ...
  const narrowTheme=await themeDropdown.evaluate(e=>{const r=e.getBoundingClientRect(),list=e.querySelector('.tp155-theme-list');return {left:r.left,right:r.right,width:r.width,viewport:innerWidth,listClient:list.clientWidth,listScroll:list.scrollWidth}});
  assert.ok(narrowTheme.left>=0&&narrowTheme.right<=narrowTheme.viewport+0.5,'inline theme chooser must fit a 320px viewport: '+JSON.stringify(narrowTheme));assert.ok(narrowTheme.listScroll<=narrowTheme.listClient+1,'theme columns must not create horizontal overflow at 320px');
  await page.setViewportSize({width:393,height:720});await page.waitForTimeout(30);
  assert.equal(await themeDropdown.evaluate(e=>getComputedStyle(e).position),'relative','theme chooser must remain attached to the Settings card');
  const themeBefore=await page.evaluate(()=>rf200ThemeKey());await themeDropdown.locator('.tp155-theme-option:not(.active)').first().click();await page.waitForTimeout(30);assert.notEqual(await page.evaluate(()=>rf200ThemeKey()),themeBefore,'Theme must be selectable while Settings remains open');assert.equal(await page.locator('#tp155R4PanelHost[data-panel="settings"]').count(),1,'changing theme must keep Settings open');assert.equal(await page.locator('.tp162-theme-dropdown[open]').count(),1,'inline theme chooser must remain usable after applying a theme');
  await page.locator('.tp162-theme-dropdown .tp155-theme-open').click();await page.waitForFunction(()=>document.querySelector('.tp162-theme-dropdown')?.open===false);
