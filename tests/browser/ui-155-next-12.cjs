...
(async()=>{
 ...
 try{
  ...
  assert.ok(Math.abs(closeReference.width-closeReference.height)<0.05,'panel close control must be square');assert.ok(closeReference.rightGap>=-0.05&&closeReference.rightGap<=24,'panel close control must stay inside the top-right panel edge: '+JSON.stringify(closeReference));assert.equal(closeReference.position,'sticky','panel close control must stay sticky');assert.ok(closeReference.contentTop>=closeReference.closeBottom-1,'panel close control must own layout space instead of covering content');assert.match(closeReference.background,/rgb\((?:6[5-9]|7[0-9]),\s*(?:3[0-9]|4[0-9]),\s*(?:3[0-9]|4[0-9])\)/,'panel close control must use the dark red treatment');
  ...
  assert.ok(panelLayers.nav>panelLayers.host,'2x4 navigation must stay above main panels');assert.ok(panelLayers.hostTop>=panelLayers.navBottom-1,'main panels must start below the 2x4 navigation');
  ...
  await themeSetting.locator('.tp155-theme-open').click();await page.waitForSelector('#tp155ThemePicker');
  const themePicker=page.locator('#tp155ThemePicker');assert.ok(await themePicker.locator('.tp155-theme-option').count()>=10,'theme picker must expose the complete scrollable list');
  assert.match(await themePicker.locator('.tp155-theme-list').evaluate(e=>getComputedStyle(e).overflowY),/auto|scroll/,'theme choices must use a scrollable list');
  const themeLayer=await page.evaluate(()=>{const m=document.getElementById('tp155ThemePicker'),h=document.getElementById('tp155R4PanelHost'),n=document.querySelector('.top.tp154-nav-grid'),card=m?.querySelector('.tp155-theme-picker'),r=card?.getBoundingClientRect();return {position:getComputedStyle(m).position,z:parseInt(getComputedStyle(m).zIndex)||0,hostZ:parseInt(getComputedStyle(h).zIndex)||0,navZ:parseInt(getComputedStyle(n).zIndex)||0,top:r?.top,bottom:r?.bottom}});assert.equal(themeLayer.position,'fixed','Theme picker must be a true viewport modal');assert.ok(themeLayer.z>themeLayer.hostZ&&themeLayer.z>themeLayer.navZ,'Theme picker must render above Settings and the 2x4 navigation');assert.ok(themeLayer.top>=0&&themeLayer.bottom<=720,'Theme picker card must be visible inside the phone viewport');
  const themeBefore=await page.evaluate(()=>rf200ThemeKey());await themePicker.locator('.tp155-theme-option:not(.active)').first().click();await page.waitForTimeout(30);assert.notEqual(await page.evaluate(()=>rf200ThemeKey()),themeBefore,'Theme must be selectable while Settings remains open');assert.equal(await page.locator('#tp155R4PanelHost[data-panel="settings"]').count(),1,'changing theme must keep Settings open');assert.equal(await themePicker.count(),1,'Theme picker must remain usable after applying a theme');
  assert.equal(await page.evaluate(()=>TrainPilotAndroidBack()),true,'Android Back must close the theme picker first');assert.equal(await themePicker.count(),0);assert.equal(await page.locator('#tp155R4PanelHost[data-panel="settings"]').count(),1);
  assert.equal(await page.evaluate(()=>TrainPilotAndroidBack()),true,'Android Back must handle an open main panel');
  assert.equal(await page.locator('#tp155R4PanelHost').count(),0,'Android Back must close the main panel');

  const scrollProbe=await page.evaluate(()=>{go('programs');const main=document.querySelector('#app main'),spacer=document.createElement('div');spacer.id='tp155ScrollProbe';spacer.style.height='1800px';main.appendChild(spacer);window.scrollTo(0,600);return window.scrollY});await page.waitForTimeout(30);await page.evaluate(()=>document.querySelector('.tp154-settings-action')?.click());await page.waitForSelector('#tp155R4PanelHost[data-panel="settings"]');const scrollWhileOpen=await page.evaluate(()=>{const n=document.querySelector('.top.tp154-nav-grid').getBoundingClientRect(),h=document.getElementById('tp155R4PanelHost').getBoundingClientRect();return {navVisible:n.bottom>0,hostBelow:h.top>=n.bottom-1}});assert.equal(scrollWhileOpen.navVisible,true);assert.equal(scrollWhileOpen.hostBelow,true,'main panel must still start below navigation when opened from a scrolled page');await page.evaluate(()=>document.querySelector('#tp155R4PanelHost[data-panel="settings"] .tp155-r4-panel-close')?.click());await page.waitForTimeout(30);const scrollAfterClose=await page.evaluate(()=>window.scrollY);assert.ok(Math.abs(scrollAfterClose-scrollProbe)<=5,'closing a main panel must preserve the background scroll position within browser pixel rounding: '+JSON.stringify({scrollProbe,scrollAfterClose}));await page.evaluate(()=>document.getElementById('tp155ScrollProbe')?.remove());
  ...
  assert.equal(await page.locator('.tp155-history-delete').count(),1,'logged workout must expose a delete action');assert.equal((await page.locator('.tp155-history-delete').first().textContent()).trim(),'Törlés');
