...
(async()=>{
 ...
 try{
  ...
  const backupTopBefore=await page.locator('.tp152-settings-backup').evaluate(e=>e.getBoundingClientRect().top);
  ...
  const backupTopOpen=await page.locator('.tp152-settings-backup').evaluate(e=>e.getBoundingClientRect().top);
  assert.ok(Math.abs(backupTopOpen-backupTopBefore)<=1,'opening Theme must not push Backup or lower Settings cards down: '+JSON.stringify({backupTopBefore,backupTopOpen}));
