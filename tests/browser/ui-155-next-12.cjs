...
(async()=>{
 ...
 try{
  ...
  assert.ok(Math.abs(closeReference.width-closeReference.height)<0.05,'panel close control must be square');assert.ok(closeReference.rightGap>=-0.05&&closeReference.rightGap<=24,'panel close control must stay inside the top-right panel edge: '+JSON.stringify(closeReference));assert.equal(closeReference.position,'absolute','1.6.1 panel close control must float without reserving a blank top row');assert.ok(closeReference.contentTop<closeReference.closeBottom,'1.6.1 panel content should start beside/under the floating close control instead of reserving its full height');assert.match(closeReference.background,/rgb\((?:6[5-9]|7[0-9]),\s*(?:3[0-9]|4[0-9]),\s*(?:3[0-9]|4[0-9])\)/,'panel close control must use the dark red treatment');
