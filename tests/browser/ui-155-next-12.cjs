...
(async()=>{
 ...
 try{
  ...
  assert.ok(narrowTheme.left>=0&&narrowTheme.right<=narrowTheme.viewport+0.5,'theme picker must fit a 320px viewport: '+JSON.stringify(narrowTheme));assert.ok(narrowTheme.listScroll<=narrowTheme.listClient+1,'theme columns must not create horizontal overflow at 320px');
