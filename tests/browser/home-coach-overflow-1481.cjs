...
(async()=>{
 ...
 try{
  ...
  assert.ok(enLayout.paddingBottom>=88,'English Home must retain safe bottom scroll space');
  assert.ok(enLayout.scrollHeight>=enLayout.viewport,'English Home must remain vertically scrollable');
